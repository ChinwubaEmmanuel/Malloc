...
// \TODO Put your Best Fit code in this #ifdef block
   ...
   int winning_space = INT_MAX;
   curr = heapList;
   while (curr) 
   {
      if(curr->free && (int)(curr->size - size) < winning_space)
      {
         winning_space = (int)(curr->size - size);
         winner = curr;
      }
      *last = curr;
      curr  = curr->next;
...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   while (curr && !(curr->free && curr->size >= size) && curr->next != last_alloc) 
   //while (curr && !(curr->free && curr->size >= size)) 
   {
      *last = curr;
      curr  = curr->next;  
...
void *malloc(size_t size) 
{
   ...
   */
   ...
   {
      struct _block *new = NULL;
      size_t remainder = (int)(next->size - size) - (sizeof(struct _block)); 
      if((int)remainder >= sizeof(struct _block) + 4)
      {
         num_splits++;
         new = BLOCK_DATA(next)-(size/sizeof(struct _block));
         //next->size = next->size - remainder;
         next->size = size;
         new->size = remainder;
         new->free = true;
         new->next = next->next;
   ...
   /*
   split 4
   split causes wf to seg fault
   next fit not working
   coalesce? 3
   statistics - max size of heap, reuse?
...
void free(void *ptr) 
{
   ...
   */
   ...
   while (curr)
   {
      ...
      {
         num_coalesces++;
         size_t prev_size = curr->next->size;
         curr->next = curr->next->next;
         curr->size = curr->size + sizeof(struct _block) + prev_size;
      }
      curr = curr->next;
   }
}
